<x-default-layout>
    ...
    @php
        $defaultAnswers = [['text' => '', 'is_correct' => false], ['text' => '', 'is_correct' => false]];
        $answers = old('answers', $defaultAnswers);
        $questionType = old('question_type', 'mcq');
    @endphp

    <form action="{{ route('admin.programs.courses.quizzes.questions.store', [$program, $course, $quiz]) }}" method="POST" id="questionForm">
        @csrf
            ...
            <x-slot:main>
                <x-forms.card-section :title="__('Question Type')">
                    <div class="mb-5">
                        ...
                        <div class="form-text">{{ __('Select the type of question') }}</div>
                    </div>
                    ...
                    <x-forms.card-section :title="__('Correct Answer')">
                        <div class="form-check form-check-custom form-check-solid mb-3">
                            <input class="form-check-input"
                                   ...
                                   {{ old('correct_answer') === '1' || old('correct_answer') === 1 || old('correct_answer') === true ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-custom form-check-solid">
                            <input class="form-check-input"
                                   ...
                                   value="0"
                                   id="false_answer"
                                   {{ old('correct_answer') === '0' || old('correct_answer') === 0 || old('correct_answer') === false ? 'checked' : '' }}>
