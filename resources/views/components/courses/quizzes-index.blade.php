{{--
 * Course Quizzes Index Component (DataTable Version)
 *
 * Displays a list of course quizzes with server-side pagination.
 * Shared between admin and instructor views with context-aware routing.
 *
 * @param string $context - 'admin' or 'instructor'
 * @param \App\Models\Program $program
 * @param \App\Models\Course $course
 * @param array $stats - ['total' => int, 'pending' => int, 'graded' => int]
--}}
...
@php
    ...
    $quizzesRoute = route("{$routePrefix}.quizzes.index", [$program, $course]);
    $courseRoute = $isAdmin
...
<div class="card mb-6">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-start">
            <div>
                <h1 class="fs-2 fw-bold mb-2">{{ __('Course Quizzes & Exams') }}</h1>
                <div class="text-muted">
                    {{ __('Manage quizzes and exams for :course', ['course' => $course->title]) }}
                </div>
            </div>
        ...
        <div class="d-flex flex-wrap pt-5">
            <div class="border border-gray-300 border-dashed rounded min-w-100px py-3 px-4 me-6">
                ...
                <div class="fw-semibold fs-7 text-gray-500">{{ __('Total') }}</div>
            </div>
            <div class="border border-gray-300 border-dashed rounded min-w-100px py-3 px-4 me-6">
        ...
        <div class="d-flex flex-wrap gap-3 align-items-center">
            <div>
                <select name="filter" class="form-select form-select-sm" data-filter="filter">
                    <option value="all">{{ __('All Quizzes') }}</option>
                    <option value="quiz">{{ __('Quizzes Only') }}</option>
                    <option value="exam">{{ __('Exams Only') }}</option>
                    <option value="published">{{ __('Published') }}</option>
                    <option value="unpublished">{{ __('Unpublished') }}</option>
                    <option value="pending">{{ __('Has Pending') }}</option>
                </select>
            </div>
            ...
            <table id="quizzes-table"
                ...
                <thead>
                    <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th class="min-w-80px text-center">{{ __('Questions') }}</th>
                        <th class="min-w-80px text-center">{{ __('Points') }}</th>
                        <th class="min-w-100px text-center">{{ __('Attempts') }}</th>
                        <th class="min-w-80px text-center">{{ __('Status') }}</th>
...
<script src="{{ asset('assets/js/custom/courses/quizzes-table.js') }}"></script>
@endpush
