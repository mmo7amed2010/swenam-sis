{{--
 * Course Assignments Index Component (DataTable Version)
 *
 * Displays a list of course assignments with server-side pagination.
...
@php
    ...
    $assignmentsRoute = route("{$routePrefix}.assignments.index", [$program, $course]);
    $courseRoute = $isAdmin
...
<div class="card mb-6">
    <div class="card-body">
        <div class="d-flex justify-content-between align-items-start">
            <div>
                <h1 class="fs-2 fw-bold mb-2">{{ __('Course Assignments') }}</h1>
                <div class="text-muted">
                    {{ __('Manage assignment submissions for :course', ['course' => $course->title]) }}
                </div>
            </div>
        ...
        <div class="d-flex flex-wrap gap-3 align-items-center">
            <div>
                <select name="filter" class="form-select form-select-sm" data-filter="filter">
                    <option value="all">{{ __('All Assignments') }}</option>
...
<script src="{{ asset('assets/js/custom/courses/assignments-table.js') }}"></script>
@endpush
